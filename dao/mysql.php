<?php
	...
	function get_comments_for_chapter($id) {
		...
		mysqli_set_charset($link, "UTF8");
		// TODO: Error check?
		$query = "SELECT c.id, u.email, u.username, u.avatar, c.chapter_id, c.text, c.date, c.paragraph, u.permissions FROM comment c, user u where u.email = c.author AND c.chapter_id = " . mysqli_real_escape_string($link, $id) . " order by c.date";
		$result = mysqli_query($link, $query);
